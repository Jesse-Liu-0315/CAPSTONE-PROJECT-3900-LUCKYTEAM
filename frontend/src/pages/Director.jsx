...
const DirectorDetail = () => {
  const directorName = useParams().directorName.toString();
  const directorNameWithSpace = directorName.replaceAll('_', ' ');
  const directorId = useParams().directorId.toString();
  ...
  React.useEffect(() => {
    ...
    try {
      makeRequest(`/director?directorID=${body.directorID}`, 'GET').then(data => {
        setCover(data.director['director_cover']);
        setDescription(data.director['director_description']);
        setBirth(data.director['director_birth']);
        setNation(data.director['director_nationality']);
        setViews(data.director['director_views']);
  ...
  return (
    <>
      ...
      <ShareModal location={'director'} cover={cover} shareModalOpen={shareModalOpen} setShareModalOpen={setShareModalOpen} 
      ...
      <div className="container-fluid">
        <div>
          ...
          <h1 style={{ color: '#1565c0' }}>{directorNameWithSpace}</h1>
        </div>
        <div className="row">
          <div className="card mb-3" style={{ maxWidth: '100vw', border: '0px' }}>
            <div className="row g-0">
              <div className="col-auto">
                <img src={cover} className="img-fluid" alt="Director" style={{ height: '235px' }} />
              </div>
              <div className="col">
                <div className="card-body" style={{ position:'relative', height: '100%', padding: '0px', paddingLeft: '5px' }}>
                  ...
                  <Button size='small' variant="outlined" onClick={handleShareModalOpen} style={{ margin: '0px', position: 'absolute', bottom: 0, left: 350 }}>Share this director</Button>
                </div>
              </div>
            </div>
          </div>
