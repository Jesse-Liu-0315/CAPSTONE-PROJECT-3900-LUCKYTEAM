...
import OtherPeopleWatchedListMovie from '../components/OtherPeopleWatchedListMovie';
import makeRequest from '../helpers/Fetch';
import { useParams } from 'react-router-dom';
import Button from '@mui/material/Button';

const OtherWatchedList = () => {
  ...
  React.useEffect(() => {
    ...
    try {
      makeRequest(`/watchedlist/other?user_id=${body.user_id}`, 'GET').then(data => {
    ...
    <>
      ...
      <div className='row'>
        <div className='col'>
            ...
            <h1 style={{ color: '#1565c0' }}>Watched list of {userName}</h1>
        </div>
        <div className='col'>
      ...
      {watchedResult.map((result, index) => {
        return <OtherPeopleWatchedListMovie key={index} 
                movieId={result.movie_id} moviePicture={result.movie_cover} movieName={result.movie_name} movieDescription={result.movie_description} movieRating={result.movie_rating} movieReview={result.review_num}></OtherPeopleWatchedListMovie>
      })}
    </>
  );
}
 
export default OtherWatchedList;
